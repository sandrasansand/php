<!DOCTYPE html>
 ...
 <?php include ('plantilla/head.php');?>
  ...
  $con=conectar_db();
    if (isset($_POST['id_txt'])){
     ...
     $nombre= $_POST['nombre'];
     $precio= $_POST['precio'];
     $stock= $_POST['stock'];//aqui pasarle tb el stock que llega desde detalle.php para hacer comprobación de que esté en stock y coincida con bd
     $cantidad= $_POST['cantidad'];//if($stock <> 0){puedo comprar es disponible $pos<>-1}
    
     $mi_carrito[]= array('id' =>$id,'nombre' =>$nombre,'precio' =>$precio, 'stock'=>$stock,'cantidad' =>$cantidad);
  ...
  if (isset($_SESSION['carrito'])){  //le preguntamos si la variable de sesion carrito existe
   $mi_carrito= $_SESSION['carrito']; //guardamos en la variable mi_carrito con los datos de Sesion que es un array de los datos del carrito
    if(isset( $_POST['id_txt'])){
      ...
      $pos=-1; //si tengo -1 el producto no existe /////aqui lo haré con el stock
      if ($stock <> 0 && $stock >= $cantidad) { //stock debe ser siempre mayor que cantidad hacer que si solo tengo uno pues solo pueda comprar como máximo la cantidad de stock
      ...
      if($pos<>-1){   //si posición es distinto a -1 es porque el producto existe. Si existe sigfca que recorrimos el bucle anterior
          $cuanto= $mi_carrito[$pos]['cantidad']+$cantidad;  //nos guardamos la cantidad a comprar pulsada hasta el momento (que serÃ¡ +1) en la variable cuanto
 ...
 if (isset($_POST['id2'])){
  ...
  $mi_carrito[$indice]['cantidad']=$cuanto; //AQUI STOCK
